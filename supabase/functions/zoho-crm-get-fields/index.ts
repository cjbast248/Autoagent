...
async function refreshToken(supabase: any, user_id: string, refresh_token: string, client_id: string, client_secret: string, zoho_region: string = 'eu') {
  console.log('[Zoho Get Fields] Refreshing token for user:', user_id);
  ...
  if (tokenData.access_token) {
    console.log('[Zoho Get Fields] Successfully refreshed token');
    await supabase
  ...
  console.error('[Zoho Get Fields] Failed to refresh token:', tokenData);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`Getting fields for module: ${module}, user: ${user_id}`);
    ...
    // Get fields from Zoho API
    let response = await fetch(
      `https://${apiDomain}/crm/v2/settings/fields?module=${module}`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ error: data.message || 'Failed to get fields' }),
        { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Transform fields to simpler format
    ...
    console.log(`Found ${fields.length} fields for module ${module}`);

    return new Response(
      JSON.stringify({ fields, module }),
